...
- Create a class named `OrderCounterResource`
...
- Create a `PricesClient` interface
...
- Create a REST endpoint of type POST with path `/orders` in the `OrderCounterResource` class that will receive an `Order`.
...
- Create a class `OrderPersister` with the following annotations: `@ApplicationScoped` and `@ActivateRequestContext`
...
- Add the following line to the order method (use the slf4j Logger for this)
```java
logger.info("Sending Order for Customer '{}' and Beverage '{}'", order.customerName, order.beverage.name());
```
Send an order without a beverage. You should now see a NullPointerException in the application because the beverage could not be found.
...
- Create a class `OrderEntity` which extends the `PanacheEntity` class and make it a JPA entity.
...
- Create a `Barista` class with the following annotations: `@ApplicationScoped` and `@ActivateRequestContext`
